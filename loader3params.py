...
class TextImageDataset(Dataset):
    ...
    def get_para(self, ind):
        ...
        paramcache=np.array([float(paramfilename.split('/')[-1].split('_')[5]),np.log10(float(paramfilename.split('/')[-1].split('_')[6])),float(paramfilename.split('/')[-1].split('_')[7])]) #image file names: *_param2_param1*.npy
        print(paramcache)
        paramcache=(paramcache-np.array([4.,1.,1.]))/np.array([2.,1.398,1.]).astype(np.float32) #data normalized to [0,1], original parameter range: [4,6] and [1,2.398]
        return paramcache
