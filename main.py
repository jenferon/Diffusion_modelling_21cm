...
def train_eor():
    ...
    run_name='test_LHS' # the unique name of each experiment
    ...
    save_model = False
    save_dir = './outputs/'
    save_freq = 10 # the period of saving model
    ...
    test_param=torch.tensor([[0.2,0.80000023],[0.351,0.340],[0.4,0.8]])
    n_sample =  64 #2, the number of samples in sampling process
    ...
    data_dir = './train_set_LHS' # data directory
    ...
    if resume:
        ddpm.load_state_dict(torch.load(os.path.join(save_dir, "model_epoch_10_test_test01.tar")), strict=False)
    ...
    #rain_size = int(.9 * len(dataset))
    #al_size = int(0.1 * len(dataset))
    #test_size = len(dataset) - train_size - val_size
    #rain_dataset, val_dataset = torch.utils.data.random_split(dataset, [train_size, val_size])
    # data loader setup
    ...
    """aloader_val = torch.utils.data.DataLoader(
        val_dataset,
        batch_size=n_sample,
        shuffle=True,
        num_workers=0,
        ...
        for i,(x, c) in enumerate(pbar):
            ...
            if ep%sample_freq==0:
                ...
                with torch.no_grad():
                    ...
                    for t in test_param: 
                        print(t)
                        #t = torch.tile(t,(n_sample,1)) # repeat to perform multiple sampling
                        ...
                        x_gen, _ = ddpm.sample(nn_model,n_sample, (1,image_size,image_size), device, test_param=t, guide_w=ws_test[0])
                        x_gen_ema, _ = ddpm.sample(ema_model,n_sample, (1,image_size,image_size), device, test_param=t, guide_w=ws_test[0])
                        ...
                        """
                        #calculate FSD for each checkpoint
                        val_features, val_labels = next(iter(dataloader_val))
                        val_features = torch.squeeze(val_features)
                        val_labels = torch.squeeze(val_labels)
                        # Convert target_row to a tensor
                        # Create a boolean mask where rows match the target_row
                        match_mask = (val_labels == test_param_single).all(dim=1)
                        # Get the indices where the mask is True (where the rows match)
                        indices = match_mask.nonzero(as_tuple=False)
                        
                        FSD_class = calc_FSD.FSD(torch.squeeze(val_features[indices]), torch.squeeze(torch.from_numpy(x_gen_tot))[indices])
                        FSD = FSD_class.calc_FSD()
                        FSD_ema_class = calc_FSD.FSD(torch.squeeze(val_features[indices]), torch.squeeze(torch.from_numpy(x_gen_tot_ema))[indices])
                        FSD_ema = FSD_ema_class.calc_FSD()"""

                        #if already file for this epoch delete
                        temp_name = os.path.join(save_dir, f"train-{ep}xscale_{ws_test[0]}_test_{run_name}_test_param_{t}*")
                        print(temp_name)
                        for filename in glob.glob(temp_name):
                            os.remove(filename) 

                        sample_save_path_final = os.path.join(save_dir, f"train-{ep}xscale_{ws_test[0]}_test_{run_name}_test_param_{t}.npy")
                        np.save(str(sample_save_path_final),x_gen_tot)
                        sample_save_path_final = os.path.join(save_dir, f"train-{ep}xscale_{ws_test[0]}_test_{run_name}_test_param_{t}_ema.npy")
