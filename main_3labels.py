...
import calc_FSD 
from loader3params import TextImageDataset
...
class DDPM(nn.Module):
    ...
    def sample(self, nn_model, n_sample, size, device, test_param, guide_w = 0.0):
        ...
        if self.cond == True:
            ...
            uncond_tokens = torch.tensor(np.float32(np.array([0,0,0]))).to(device)
...
def train_eor():
    ...
    run_name='test_3params' # the unique name of each experiment
    ...
    test_param_single=torch.tensor([0.2,0.80000023,0.63])
    ...
    n_param = 3 # dimension of parameters
    data_dir = './train_set_xHI' # data directory
            ...
            if ep%sample_freq==0:
                ...
                with torch.no_grad():
                    ...
                    for w in ws_test: 
                        ...
                        temp_name = os.path.join(save_dir, f"train-{ep}xscale_{w}_test_{run_name}*")
                        print(temp_name)
